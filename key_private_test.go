...
func Test_NewPrivateKeyFromPEMBytes(t *testing.T) {
	providedRSAPrivKey, err := rsa.GenerateKey(rand.Reader, 1024)
	assert.NilError(t, err)
	providedPrivKey, err := WrapPrivateKey(providedRSAPrivKey)
	assert.NilError(t, err)

	getRSAPrivateKeyPEMBlocks := func(t *testing.T, rsaPrivKey *rsa.PrivateKey) *pem.Block {
		pemBlock := &pem.Block{
	...
	t.Run("without passphrase", func(t *testing.T) {
		privKey, err := NewPrivateKeyFromPEMBytes(pem.EncodeToMemory(getRSAPrivateKeyPEMBlocks(t, providedRSAPrivKey)), nil)
		assert.NilError(t, err)
	...
	t.Run("with passphrase", func(t *testing.T) {
		...
		getRSAPrivateKeyPEMBlocks := func(t *testing.T) *pem.Block {
			pemBlock := getRSAPrivateKeyPEMBlocks(t, providedRSAPrivKey)
			pemBlock, err := x509.EncryptPEMBlock(rand.Reader, pemBlock.Type, pemBlock.Bytes, []byte(passphrase), x509.PEMCipherAES256)
			assert.NilError(t, err)
			return pemBlock
		}
		...
		t.Run("without passphrase getter", func(t *testing.T) {
			...
			assert.Check(t, func() cmp.Result {
				...
				return cmp.ResultFromError(nil)
			})
			assert.Check(t, cmp.Nil(privKey))
		})
		...
		t.Run("with error retrieving the passphrase", func(t *testing.T) {
			privKey, err := NewPrivateKeyFromPEMBytes(pem.EncodeToMemory(getRSAPrivateKeyPEMBlocks(t)),
				func() ([]byte, error) { return nil, anError },
			)
...
func Test_privateKey_Equal(t *testing.T) {
	...
	t.Run("ko", func(t *testing.T) {
		...
		t.Run("two different keys are not equal", func(t *testing.T) {
			rawPrivKey1, err := rsa.GenerateKey(rand.Reader, 1024)
			assert.NilError(t, err)
			rawPrivKey2, err := rsa.GenerateKey(rand.Reader, 1024)
			assert.NilError(t, err)
...
func cmpPrivateKey(x PrivateKey, y PrivateKey) cmp.Comparison {
